...
export default class DatGUI {
  constructor(main, mesh) {
    ...
    const cameraPositionXGui = cameraFolder.add(Config.camera, 'posX', -500, 500).name('Position X');
    cameraPositionXGui.onChange((value) => {
      this.camera.position.x = value;
    });
    const cameraPositionYGui = cameraFolder.add(Config.camera, 'posY', -500, 500).name('Position Y');
    cameraPositionYGui.onChange((value) => {
      this.camera.position.y = value;
    });
    const cameraPositionZGui = cameraFolder.add(Config.camera, 'posZ', -500, 500).name('Position Z');
    cameraPositionZGui.onChange((value) => {
